...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
#define  s2_i_log0    PORTC &= ~_BV(PORTC2); dly0();   // (0/1)   :  s2_i
#define  s2_i_log1    PORTC |=  _BV(PORTC2); dly0();   // (0/1)   :  s2_i
#define  s1_i_log0    PORTC &= ~_BV(PORTC1); dly0();   // (0/1)   :  s1_i
#define  s1_i_log1    PORTC |=  _BV(PORTC1); dly0();   // (0/1)   :  s1_i
#define  rb_i_log0    PORTC &= ~_BV(PORTC0); dly0();   // (0/1)   :  rb_i
#define  rb_i_log1    PORTC |=  _BV(PORTC0); dly0();   // (0/1)   :  rb_i
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
...
// RE-init Data-Direction-Registers and the output of PORTC
  {
   DDRC |=  _BV(DDC0); PORTC |=  _BV(PORTC0); // DDC0:out,PORTC0: FPGA_rb_i=1
   DDRC |=  _BV(DDC1); PORTC |=  _BV(PORTC1); // DDC1:out,PORTC1: FPGA_s1_i=1
   DDRC |=  _BV(DDC2); PORTC |=  _BV(PORTC2); // DDC2:out,PORTC2: FPGA_s2_i=1
   DDRC &= ~_BV(DDC3); PORTC |=  _BV(PORTC3); // DDC3: in,PORTC3:  uC_txd_i=pup
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
   CRLF(); Fmsg(m_ardFPGA); CRLF(); FPGA_Help();
//-----------------------------------------------------------------------------
   while (mode!='Q')
   {CRLF();
    TTYo('<');TTYo('?');rxd0=TTYw();TTYo(rxd0);TTYo('=');
//-----------------------------------------------------------------------------
    switch (rxd0)
    {
//-----------------------------------------------------------------------------
     case 'R':                                           // Reset to FPGA
     {TTYo('0');TTYo('/');TTYo('1');TTYo('?');TTYo('>'); // choice is 0 or 1
      TTYo('R');TTYo('B');TTYo('=');
      rxd0=TTYw();                                       // enter selection
      if ( (rxd0=='0')|(rxd0=='1') )                     // 0 or 1 is ok
      {TTYo(rxd0);
       if (rxd0=='0'){ rb_i_log0;}                       //  rb: log0 
       if (rxd0=='1'){ rb_i_log1;}                       //  rb: log1
      }
      else {TTYo('?');TTYo('?');}                        // invalid choice
      rxd0=0x00; break;
     }
//-----------------------------------------------------------------------------
     case 'A':                                           // WRITE BIT s1
     {TTYo('0');TTYo('/');TTYo('1');TTYo('?');TTYo('>'); // choice is 0 or 1
      ...
      if ( (rxd0=='0')|(rxd0=='1') )                     // 0 or 1 is ok
       {TTYo(rxd0);
        if (rxd0=='0'){s1_i_log0;}                       // s1: log0
        if (rxd0=='1'){s1_i_log1;}                       // s1: log1
       }
      else {TTYo('?');TTYo('?');}                        // invalid choice
      rxd0=0x00; break;
     }
//-----------------------------------------------------------------------------
     ...
     {TTYo('0');TTYo('/');TTYo('1');TTYo('?');TTYo('>'); // choice is 0 or 1
      ...
      if ( (rxd0=='0')|(rxd0=='1') )                     // 0 or 1 is ok
       {TTYo(rxd0);
        if (rxd0=='0'){s2_i_log0;}                       // s2: log0
        if (rxd0=='1'){s2_i_log1;}                       // s2: log1
       }
      else {TTYo('?');TTYo('?');}                        // bad choice
...
//-----------------------------------------------------------------------------
     ...
     {s1_i_log1;                          // Set S1 = log0
      ...
      CRLF(); Fmsg(m_ardexc5);            // Message to TTY
      CRLF(); Fmsg(m_ardexc6);            // Message to TTY
      CRLF(); Fmsg(m_ardexc7);            // Message to TTY
      s1_i_log0;                          // S1=log1, Measurement starts
      rxd0=TTYw();TTYo(rxd);              // hit any key to stop
      ...
      CRLF(); Fmsg(m_ardexc8);            // we are done
      s1_i_log1;                          // Set S1 = log0
      s2_i_log1;                          // Set S2 = log0
      rxd0=0x00; break;
     }
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
     case 'H': {rxd0='?';}                                 // H: HELP-message
     case '?': {CRLF(); FPGA_Help();rxd0=0x00; break;}     // ?: HELP-message
//-----------------------------------------------------------------------------
     case 'Q': {CRLF(); mode='Q';rxd0=0x00; break;}        // Q: QUIT
//-----------------------------------------------------------------------------
     default : {if (rxd0!=0x00){TTYo('>');TTYo('?');TTYo('?');CRLF();}
     break;
               }
//-----------------------------------------------------------------------------
